...
public class ImageServiceImpl implements ImageService {
     ...
     * In this example, neither of these options are used, which means the
     * following code only works when the credentials are defined via the
     * environment variable GOOGLE_APPLICATION_CREDENTIALS, and those credentials
     * are authorized to sign a URL. See the documentation for Storage.signUrl for
     * more details.
     *
     * @throws IOException
     *
    ...
    public ImageDTO generateV4PutObjectSignedUrl(String objectName,
        ...
        if (fileExtension == null || (!fileExtension.equalsIgnoreCase("vmdk") &&
                !fileExtension.equalsIgnoreCase("vhd"))) {
            return null;
        }
        ...
        credentials = ImpersonatedCredentials.create(
                (GoogleCredentials) credentials, serviceAccount, new ArrayList<String>(),
                scopes, 3600);

        // Generate Signed URL
        Map<String, String> extensionHeaders = new HashMap<>();
        extensionHeaders.put("Content-Type", "application/octet-stream");
        extensionHeaders.put("x-goog-meta-os", operatingSystem);
    ...
    /**
     * Start a build to convert the uploaded virtual disk to an image that can be
     * used by a compute instance. The build process is long and may be 30 minutes
     * or more. Object name is the name of the uploaded virtual disk, while
     * imageName is the name of the image given by the user.
     *
     ...
     */
    @Override
    public BuildImageDTO startVirtualDiskBuild(String objectName,
            String imageName)
        ...
        try (CloudBuildClient cloudBuildClient = CloudBuildClient.create()) {
            BuildStep buildStep = BuildStep.newBuilder()
                    ...
                    .addArgs(String.format(
                            "-source_file=%s",
                            String.format("gs://%s/%s", bucketName, objectName)))
            ...
            OperationFuture<Build, BuildOperationMetadata> operation = cloudBuildClient
                    .createBuildAsync(createBuildRequest);
    ...
    /**
     * Cancel an ongoing build that converts a virtual disk to an image that can
     * be used by a compute instance.
     *
     * @param buildId
     * @return response
     */
    @Override
    public BuildImageDTO cancelVirtualDiskBUild(String buildId)
            throws IOException {
    ...
    public List<ImageDTO> listImages() throws IOException {
        ...
        try (ImagesClient imagesClient = ImagesClient.create()) {
            ...
            // Although the `setMaxResults` parameter is specified in the request, the
            // iterable returned by the `list()` method hides the pagination mechanic.
            // The library makes multiple requests to the API for you, so you can
            // simply iterate over all the images.
            ...
            for (Image image : imagesClient.list(imagesRequest).iterateAll()) {
                ...
                imageDTO.setCreationTimestamp(
                        Instant.parse(image.getCreationTimestamp()));
     ...
     */
    @Override
    public DeleteImageDTO deleteImage(String imageName)
        ...
        try (ImagesClient imagesClient = ImagesClient.create()) {
            ...
            deleteImageDTO.setDeleteStatus(
                    DeleteImageStatus.valueOf(response.getStatus().name()));
